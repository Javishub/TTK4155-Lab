...
static int scale_encoder_val(int value){
	return value/(MAX_ENCODER_VAL-MIN_ENCODER_VAL);
}
...
void joystick_motoring(int joystick){
	int speed = 0;
	int pid_speed = abs(pid(joystick, read_motor_encoder()));
	if (joystick > 170){
		set_motor_dir(RIGHT);
		speed = map(joystick,175,255,0,300);
		pid_speed = abs(pid(joystick, read_motor_encoder()));
		printf("Joystickspeed: %d\n\r", pid_speed);
	}
	else{
		set_motor_dir(LEFT);
		speed = map(joystick,0,165,300,0);
		pid_speed = abs(pid(joystick, read_motor_encoder()));
		printf("Joystickspeed: %d\n\r", pid_speed);
	}
	//printf("SPEEED: %d\n\r", speed);
...
void joystick_motoring(int joystick){
	...
	int pos = ((map(joystick, 0, 162,MIN_ENCODER_VAL,MAX_ENCODER_VAL/2) + map(joystick, 163, 230,(MAX_ENCODER_VAL/2)+1,MAX_ENCODER_VAL))/2);
	if(pos < 0){
		pos = 0;
	}
